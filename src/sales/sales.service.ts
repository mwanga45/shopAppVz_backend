    ...
    return await this.Datasource.transaction(async (manager) => {
      try {
        ...
        if (dto.UpdateType === UpdatePendingType.PaidPending) {
          ...
          if (!updatesales.affected) throw new Error('No update occured');
          const updateRawDetails = await manager.findOne(Tablename, {
            where: { id: dto.Sales_id },
          });
          if (!updateRawDetails)
          ...
          const Profitrecord = await this.Profitupdatesummary(
            manager,
            Number(updateRawDetails.Expected_Profit),
            Number(updateRawDetails.Revenue),
            dto.paymentVia,
          );
          if (!Profitrecord.success)
            throw new Error(String(Profitrecord.message));
          const CaptalRec = await this.BusinessGrowthLogic.UpdateCapital(
            manager,
            dto.paymentVia,
            Number(updateRawDetails.Revenue),
          );
          if (!CaptalRec.success) throw new Error(String(CaptalRec.message));
          return {
